...
@Test public class TC004_bids_detail_navigation extends base_sd 
{
	@BeforeMethod
	@BeforeTest
	...
	@Test()
	public void bids_detail_navigation_test() throws InterruptedException 
	{
		test = report.startTest("Home: Verify navigation to bid detail screen");
		...
		//clicks text lowest unique bid
		driver.findElement(By.id("in.interactive.luckystars:id/tv_type")).click();	
		String expected_text ="Lowest Unique Bid";	
		...
		if(expected_text.contentEquals(actual_text))
		{
			...
			test.log(LogStatus.INFO, "User navigated to bid detail  screen");
		...
		else
		{
			...
			test.log(LogStatus.INFO, "User not navigated to bid detail  screen");
