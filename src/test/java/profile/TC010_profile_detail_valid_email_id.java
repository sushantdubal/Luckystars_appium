...
import com.relevantcodes.extentreports.LogStatus;import config.base_sd;
import config.constants;
//on boarding class inherits base class
@Test public class TC010_profile_detail_valid_email_id extends base_sd 
{
	@BeforeMethod
	@BeforeTest
	...
	@Test()
	public void profile_detail_valid_email_id_test() throws InterruptedException 
	{
		test = report.startTest("Profile: Verify valid email id");
		...
		driver.navigate().back();
		//clicks email id
		driver.findElement(By.id("et_email")).click();
		driver.findElement(By.id("et_email")).clear();
		driver.findElement(By.id("et_email")).sendKeys("sushant.dubal@interactive.in");
		//click continue button
		driver.findElement(By.id("btn_continue")).click();
		//click confirm button
		driver.findElement(By.id("button1")).click();
		String expected_text ="CONTINUE";	
		String actual_text=driver.findElement(By.id("in.interactive.luckystars:id/btn_continue")).getText();
		...
		if(expected_text.contentEquals(actual_text))
		{
			...
			test.log(LogStatus.INFO, "Profile: Verify valid email id");
