...
@Test public class TC002_share_now extends base_sd 
{
	...
	@Test()
	public void share_now_test() throws InterruptedException 
	{
		test = report.startTest("Home: Verify share now button");
		...
		String expected_text ="SHARE NOW";	
		String actual_text=driver.findElement(By.id("in.interactive.luckystars:id/btn_share")).getText();
		...
		if(expected_text.contentEquals(actual_text))
		{
			...
			test.log(LogStatus.INFO, "Share now button exists");
		...
		else
		{
			...
			test.log(LogStatus.INFO, "Share now button not exists");
